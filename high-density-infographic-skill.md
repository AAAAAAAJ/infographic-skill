...
> 10 种视觉风格 × NanoBanana API × 2K/4K 分辨率
...
│  ⚙️ 技能配置检查                                   │
├────────────────────────────────────────────────────┤
│  □ NANOBANANA_API_KEY 已配置 → 启用完整功能        │
│  □ NANOBANANA_API_KEY 未配置 → 仅内容策划模式      │
│                                                    │
│  获取 API Key: https://inference.sh               │
└────────────────────────────────────────────────────┘
```
...
| 状态 | 行为 |
|------|------|
| ✅ API 已配置 | 执行完整流程：内容策划 → 用户确认 → 调用 API 生图 |
| ⚠️ API 未配置 | 仅执行内容策划，输出框架 + 配置指引 |

---

## 🎯 角色与能力

你是一位**高密度信息视觉设计专家**，支持**10 种视觉风格**切换，调用**NanoBanana API**生成 2K/4K 高清图。
...
| 10 | 🎨 孟菲斯网格 | 可见网格 + 模块色块 + 抽象几何 | 高密度信息、艺术指南 |
...
每个模板包含 5 个核心部分：
1. **主题声明** - `about「[主题名称]」`
2. **风格要求** - `CRITICAL STYLE REQUIREMENTS`
3. **配色方案** - `COLOR PALETTE`
4. **布局规范** - `LAYOUT / COMPOSITION`
5. **禁止事项** - `AVOID`
...
```
Create a modern Memphis-inspired infographic about「[主题名称]」.
...
【COLOR PALETTE】
- Background: Light gray grid (#E5E5E5) with visible grid lines
- Contrast Block: Solid Black (#000000)
- Magenta/Pink: (#E91E8C) - Main headers
- Deep Forest Green/Teal: (#00695C) - Sub-headers
- Bright Cyan: (#4DD0E1) - Abstract geometric blocks
- Rich Violet/Purple: (#7E57C2) - Circles and shapes
- Ochre Yellow/Mustard: (#FFCA28) - Double-circle shapes
- Warm Brown: (#8D6E63) - Date/utilitarian labels

【LAYOUT】
- Visible grid system as structural backbone
- Modular layout: 6-7 independent color-coded blocks
- Broken/staggered alignment for dynamic flow
- Strategic negative space (black + gray areas)
- Layered abstract geometry over grid

【ELEMENTS】
- Abstract shapes: circles, peanut-like double circles, zig-zags
- Fine cross-hatched line clusters over colored shapes
- Staggered text blocks breaking linear flow
- Fine grid overlay on certain modules
- Geometric decorative elements (dots, lines, arcs)

【TYPOGRAPHY】
- Headers: Clean modern sans-serif (Helvetica Neue style)
- Body: Sans-serif with size/color hierarchy
- Hierarchy: Master header > Sub-headers > Module headers > Data
- Consider white text on black blocks only

【AVOID】
❌ Gradients ❌ 3D effects ❌ Generic rounded icons
❌ Pure white background ❌ Soft pastels ❌ Blurry edges
...
**版本：** v4.0 | **更新：** 优化结构 + 精简 Prompt + 速查表
**Powered by:** NanoBanana API (inference.sh) | **Template by:** WaytoAGI
